...
- **v3: GRPO via Hermes RL stack** — explicit RL optimization with domain physics gates as the reward function. Two existing GRPO stacks available on local infrastructure (MetaClaw and Hermes, see RL-Comparison-Report in Obsidian vault). Hermes is the right backend: programmatic verification (deterministic, ground-truth) vs MetaClaw's LLM-as-judge (soft, probabilistic). Integration path: subclass Hermes's `AgenticOPDEnv` as `DetrixPhysicsEnv`, wire domain physics gates as `compute_reward()`, use Tinker for cloud LoRA. Only if v1-v2 plateau.
- **Hermes OPD for dense training signal** — Hermes's On-Policy Distillation uses hindsight from tool execution as token-level teaching signal. For Detrix: gate diagnostic output (WHY R_wp was too high, WHICH lattice parameter was out of bounds) becomes dense per-token advantages, not just scalar pass/fail. This produces richer training signal than trajectory-level reward alone.
- **MetaClaw MadMax scheduler for overnight loop** — MetaClaw's `SlowUpdateScheduler` trains during idle windows (sleep hours, keyboard inactivity, calendar events). Hot-swaps weights on completion. Borrow this pattern for Detrix's autonomous overnight improvement loop. Already built, needs integration.
- **MetaClaw skill auto-evolution for harness optimization** — MetaClaw's `SkillEvolver` analyzes failed samples, generates new skills from failure patterns, tracks generations with MAML-like separation (pre-evolution samples never enter post-evolution RL updates). Borrow for Axis 1 harness optimization: failed gate traces → automatic harness improvements. Complements Meta-Harness coding agent approach.
- **Why ART, not custom:** The training loop is commodity. OpenPipe's founder said it himself: RL-SFT via ART is the way to go. The training SIGNAL from domain-validated governance scoring is the moat, not the training loop. Don't rebuild what's Apache-2.0. v3 GRPO is available via existing Hermes stack when needed.
...
- **Training infrastructure already available** (from RL-Comparison-Report): Both MetaClaw and Hermes use Tinker for cloud LoRA with no local GPU required for GRPO. Hermes uses SGLang for inference (port 8001). Neither is currently active — requires `TINKER_API_KEY` + `WANDB_API_KEY` to activate. Hermes supports Qwen3-8B (locked), MetaClaw supports Qwen3-4B (configurable). Both use LoRA rank 32.
